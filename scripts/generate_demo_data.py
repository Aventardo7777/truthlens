...
# (5) abnormally stable monthly target series -- 'monthly_target' has constant
#     variance across the whole period (tampered spreadsheet)
month_key = dates.to_period("M").astype(str)
target_lookup = {}
for k in pd.unique(month_key):
    mnum = int(k.split("-")[1])
    base = 240000 + 800 * mnum
    target_lookup[k] = base
monthly_target = np.array([target_lookup[k] + rng.normal(0, 500) for k in month_key])
monthly_target = np.round(monthly_target, 2)

# (8) level shift: unit prices jump 40% after July 2024 (spliced data source)
shift_mask = dates > pd.Timestamp("2024-07-01")
unit_price[shift_mask] = np.round(unit_price[shift_mask] * 1.4, 2)
amount = np.round(units * unit_price, 2)
total = np.round(amount * 3.02, 2)
...
# (1)+(2) duplicates: append copied blocks (order_id keeps its original value,
# exactly like a real copy-paste expansion would)
block = df.iloc[600:660].copy()          # 60-row verbatim block
scattered = df.sample(70, random_state=7).copy()  # scattered exact dupes
tampered = pd.concat([df, block, scattered], ignore_index=True)
tampered = tampered.sort_values("order_date").reset_index(drop=True)
