...
def run_temporal_scan(df: pd.DataFrame, datetime_cols: list[str]) -> tuple[list[Finding], dict]:
    ...
    for col in num_cols[:10]:
        ...
        # sudden level shift: change-point style -- compare the mean of the
        # window before a point with the window after it (robust MAD scale)
        n = len(y)
        w = min(max(10, n // 15), 200)
        med = y.median()
        mad = float((y - med).abs().median() * 1.4826) or float(y.std())
        if mad > 0 and n > 2 * w + 10:
            diffs: list[float] = []
            positions: list[int] = []
            for i in range(w, n - w, max(1, w // 2)):
                left = y.iloc[i - w:i].mean()
                right = y.iloc[i:i + w].mean()
                diffs.append(abs(float(left - right)))
                positions.append(i)
            if diffs:
                max_jump = max(diffs)
                jump_ratio = max_jump / mad
                # also require this jump to stand out from its own distribution
                med_diff = float(np.median(diffs)) or 1e-9
                if jump_ratio > 1.5 and max_jump > 4 * med_diff:
                    pos = positions[int(np.argmax(diffs))]
                    findings.append(_mk(fid,
                        ...
                        stat_summary=(
                            f"跳变点前后窗口的均值差达到稳健尺度的 {jump_ratio:.1f} 倍，"
                            f"发生在 {ts_sorted.iloc[min(pos, len(ts_sorted) - 1)]} 附近。"
                        ),
                        evidence={
                            "column": col,
                            "jump_to_scale": round(jump_ratio, 2),
                            "robust_scale_mad": round(mad, 4),
                            "shift_magnitude": round(max_jump, 4),
                        },
                    why_it_matters=(
                        "均值水平在短期内发生远超自身波动的位移，"
