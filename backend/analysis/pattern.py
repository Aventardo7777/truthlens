...
def run_pattern_scan(df: pd.DataFrame, cat_cols: list[str] | None = None) -> list[Finding]:
        ...
        for cat in cat_cols[:5]:
            ...
            for col in num.columns:
                ...
                if np.isnan(means).any() or np.isnan(stds).any():
                    continue
                mean_cv = float(stds.mean() / abs(means.mean())) if means.mean() != 0 else np.inf
                ...
                if mean_cv < 0.01 and spread_cv < 0.05:
                    findings.append(_mk(fid,
    ...
    try:
        ...
        if len(num_only) >= 40 and num_only.shape[1] >= 2:
            ...
            for i in range(1, len(tuples)):
                if tuples[i] != tuples[i - 1]:
                    ...
                    start = i
            if len(tuples) - start >= 3:
                runs.append((start, len(tuples) - start))
            n_in_runs = sum(r[1] for r in runs)
            if len(runs) >= 2 and n_in_runs >= 0.15 * len(tuples):
                findings.append(_mk(fid,
                    category="repeated_blocks",
                    title="检测到连续重复的数据块 (Repeated Row Blocks)",
                    severity=SEVERITY_HIGH,
                    evidence_level="high",
                    stat_summary=f"{len(runs)} 段连续重复块，共覆盖 {n_in_runs} 行（{n_in_runs / len(tuples) * 100:.0f}%）。",
                    evidence={"n_blocks": len(runs), "rows_covered": int(n_in_runs),
                              "pct": round(n_in_runs / len(tuples) * 100, 1),
                              "longest_block": int(max(r[1] for r in runs))},
                    why_it_matters=(
                        "整块行连续重复且块内值完全相同，"
                        "通常出现在人工扩表场景：为达到目标行数，将一段数据反复粘贴。"
                        "这种结构对任何基于独立样本假设的统计检验都是灾难性的。"
                    ),
                    possible_causes=["人工复制粘贴扩表", "脚本循环写入错误", "数据生成时重复采样"],
                    what_it_does_not_prove="极少数场景（如按事件计数的宽表）中合法。",
                    next_steps=["定位重复块的起止行号并检查其相邻记录", "确认ETL流程中是否有循环追加逻辑"],
                ))
                fid += 1
    except Exception:
        pass
